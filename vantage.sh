...
main() {
    while :; do
        ...
        local menu="$(zenity --list --title "Lenovo Vantage" --text "Select function:" --column "Function" --column "Status" "${options[@]}" --height 970 --width 980)"
        case "$menu" in
            "Conservation Mode")
            ...
            "Always-On USB")
                ...
                ;;
            "Fan Mode")
                local submenu="$(show_submenu "Fan Mode" "$(get_fan_mode_status)" --height 400 --width 450 \
                    "Super Silent" \
                    "Standard" \
                    "Dust Cleaning" \
